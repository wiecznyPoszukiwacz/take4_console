...
describe('StatusLED', () => {
	...
	it('defaults to state "off"', () => {
		const led = new StatusLED(new Pos(0, 0));
		expect(led.getState()).toBe('off');
	});

	it('accepts initial state from options', () => {
		const led = new StatusLED(new Pos(0, 0), { state: 'ok' });
	...
	it('auto-sizes width to 2 + label.length', () => {
		const led = new StatusLED(new Pos(0, 0), { label: 'OK' });
		expect(led.getSize().width).toBe(4);   // 2 + 2
	});

	it('height is always 1', () => {
		const led = new StatusLED(new Pos(0, 0), { label: 'hello' });
		expect(led.getSize().height).toBe(1);
	});

	it('setLabel / getLabel round-trip', () => {
		const led = new StatusLED(new Pos(0, 0), { label: 'test' });
		led.setLabel('new');
	...
	it('dot is grey (240) in off state', () => {
		const led = new StatusLED(new Pos(0, 0), { state: 'off' });
		led.render();
	...
	it('dot is yellow (226) in warn state', () => {
		const led = new StatusLED(new Pos(0, 0), { state: 'warn' });
		led.render();
		expect(led.getCell(0, 0).attributes.foreground).toBe(226);
	});

	it('dot is red (196) in error state', () => {
		const led = new StatusLED(new Pos(0, 0), { state: 'error' });
		led.render();
	...
	it('label chars appear starting at column 2', () => {
		const led = new StatusLED(new Pos(0, 0), { label: 'AB' });
		led.render();
