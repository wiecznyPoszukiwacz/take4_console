...
describe('Sparkline', () => {
	...
	it('defaults to empty data', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(10, 1));
	...
	it('does not crash with a single data point', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(10, 1), { data: [5] });
		expect(() => s.render()).not.toThrow();
	});

	it('renders block-character glyphs across the row', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(8, 1), { data: [0, 25, 50, 75, 100] });
		s.render();
	...
	it('leftmost column reflects the lowest value (empty glyph)', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(8, 1), { data: [0, 50, 100] });
		s.render();
	...
	it('flat data maps to a mid-height glyph', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(6, 1), { data: [5, 5, 5, 5] });
		s.render();
	...
	it('glyph foreground matches requested color', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(6, 1), { data: [1, 2, 3], color: 196 });
		s.render();
		expect(s.getCell(0, 0).attributes.foreground).toBe(196);
	});

	it('renders on the bottom row when height > 1', () => {
		const s = new Sparkline(new Pos(0, 0), new Size(6, 3), { data: [0, 50, 100] });
		s.render();
