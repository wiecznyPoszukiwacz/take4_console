...
describe('Tabs', () => {
	...
	it('defaults to empty titles and activeIndex 0', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) });
		expect(t.getTitles()).toEqual([]);
		expect(t.getActiveIndex()).toBe(0);
	});

	it('accepts initial titles and activeIndex', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B', 'C'], activeIndex: 2 });
		expect(t.getTitles()).toEqual(['A', 'B', 'C']);
		expect(t.getActiveIndex()).toBe(2);
	});

	it('clamps initial activeIndex into range', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B'], activeIndex: 99 });
	...
	it('setActiveIndex clamps into range', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B', 'C'] });
		t.setActiveIndex(10);
		expect(t.getActiveIndex()).toBe(2);
		t.setActiveIndex(-5);
		expect(t.getActiveIndex()).toBe(0);
	...
	it('arrow keys do not wrap around', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B'] });
	...
	it('disabled Tabs ignores keys', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10), disabled: true }, { titles: ['A', 'B'] });
		t.handleKey('\x1b[C');
		expect(t.getActiveIndex()).toBe(0);
	});
	...
	it('addChildToTab adds the child and tags it with its tab index', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B'] });
		const w = new Window({ pos: new Pos(0, 1), size: new Size(5, 3) });
		t.addChildToTab(1, w);
	...
	it('renders tab titles on the header row', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['One', 'Two'] });
		t.render();
	...
	it('active tab text is bold', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['One', 'Two'], activeIndex: 1 });
		t.render();
		const { x: ox, y: oy } = t.getInnerOffset();
	...
	it('only tagged children matching active tab are composited', () => {
		const t = new Tabs({ pos: new Pos(0, 0), size: new Size(30, 10) }, { titles: ['A', 'B'] });
		const wA = new Window({ pos: new Pos(2, 2), size: new Size(3, 1) });
		const wB = new Window({ pos: new Pos(2, 2), size: new Size(3, 1) });
		wA.setCell(0, 0, 'A');
		wB.setCell(0, 0, 'B');
