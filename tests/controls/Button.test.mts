...
describe('Button', () => {
  describe('constructor / state', () => {
    it('stores the label from options', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(20, 3), label: 'OK' });
      expect(btn.getLabel()).toBe('OK');
    });

    it('defaults to empty label', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(10, 3) });
    ...
    it('setLabel / getLabel round-trip', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(20, 3) });
      btn.setLabel('Save');
  ...
  describe('render()', () => {
    ...
    it('renders the label centred inside the border', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(12, 3), label: 'Hi' });
      btn.render();
    ...
    it('focused label is bold', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(12, 3), label: 'X', focused: true });
      btn.render();
    ...
    it('disabled label is not bold', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(12, 3), label: 'X', disabled: true });
      btn.render();
    ...
    it('disabled button has dim border', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(12, 3), disabled: true });
      btn.render();
      expect(btn.getCell(0, 0).attributes.dim).toBe(true);
    });

    it('non-disabled button does not have dim border', () => {
      const btn = new Button({ pos: new Pos(0, 0), size: new Size(12, 3) });
      btn.render();
