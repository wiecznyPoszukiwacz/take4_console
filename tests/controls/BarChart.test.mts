...
describe('BarChart', () => {
	...
	it('defaults to empty data', () => {
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(20, 8) });
	...
	it('does not crash with empty data', () => {
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(10, 5) });
	...
	it('bar at 100% fills the full plot height from the bottom', () => {
		...
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(5, 5) }, { data: [100], max: 100 });
		bc.render();
	...
	it('bar at 0% is all empty (spaces)', () => {
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(5, 5) }, { data: [0], max: 100 });
		bc.render();
		for (let row = 0; row < 4; row++) {
			expect(bc.getCell(0, row).char).toBe(' ');
		}
	...
	it('filled cells have the configured barColor foreground', () => {
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(5, 5) }, { data: [100], max: 100, barColor: 82 });
		bc.render();
	...
	it('label char appears in the last row at bar column', () => {
		const bc = new BarChart({ pos: new Pos(0, 0), size: new Size(10, 5) }, { data: [50], labels: ['X'], max: 100 });
		bc.render();
