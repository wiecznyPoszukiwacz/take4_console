...
describe('ProgressBarV', () => {
	...
	it('defaults to value 0', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(3, 6));
	...
	it('clamps initial value to max', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(3, 6), { value: 999, max: 50 });
	...
	it('setValue clamps above max', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(3, 6), { max: 50 });
		pb.setValue(200);
		expect(pb.getValue()).toBe(50);
	});
	...
	it('all cells are empty chars when value is 0', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(2, 4), { value: 0 });
		pb.render();
	...
	it('all cells are filled chars when value equals max', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(2, 4), { value: 100, max: 100 });
		pb.render();
		for (let row = 0; row < 4; row++) {
			expect(pb.getCell(0, row).char).toBe('█');
		}
	...
	it('fills from the bottom upward', () => {
		...
		const pb = new ProgressBarV(new Pos(0, 0), new Size(1, 4), { value: 50, max: 100 });
		pb.render();
	...
	it('filled cells have the configured fillColor background', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(1, 2), { value: 100, fillColor: 82 });
		pb.render();
		expect(pb.getCell(0, 1).attributes.background).toBe(82);
	});

	it('empty cells have the configured emptyColor background', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(1, 2), { value: 0, emptyColor: 236 });
		pb.render();
		expect(pb.getCell(0, 0).attributes.background).toBe(236);
	});

	it('default fillColor is 75', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(1, 2), { value: 100, max: 100 });
		pb.render();
		expect(pb.getCell(0, 1).attributes.background).toBe(75);
	});

	it('fills all columns when width > 1', () => {
		const pb = new ProgressBarV(new Pos(0, 0), new Size(3, 2), { value: 100, max: 100 });
		pb.render();
