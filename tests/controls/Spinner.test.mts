...
describe('Spinner', () => {
	...
	it('defaults to braille style, frame 0, running=true', () => {
		const s = new Spinner(new Pos(0, 0));
	...
	it('accepts initial style, frame, running, label', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', frame: 2, running: false, label: 'Loading' });
	...
	it('wraps the initial frame into range', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', frame: 10 }); // line has 4 frames
		expect(s.getFrame()).toBe(2);
	});
	...
	it('auto-sizes width to glyph width when no label', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line' });
		expect(s.getSize().width).toBe(1);
	});

	it('auto-sizes width to glyph + space + label', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', label: 'Go' });
		expect(s.getSize().width).toBe(1 + 1 + 2);
	});

	it('dots style uses 3-wide frames', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'dots', label: 'wait' });
		expect(s.getSize().width).toBe(3 + 1 + 4);
	});

	it('height is always 1', () => {
		const s = new Spinner(new Pos(0, 0), { label: 'hello' });
	...
	it('step wraps at the end of the cycle', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', frame: 3 });
		s.step();
		expect(s.getFrame()).toBe(0);
	});

	it('step is a no-op when stopped', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', running: false });
		s.step();
		expect(s.getFrame()).toBe(0);
	});
	...
	it('glyph uses the requested color', () => {
		const s = new Spinner(new Pos(0, 0), { style: 'line', color: 196 });
		s.render();
