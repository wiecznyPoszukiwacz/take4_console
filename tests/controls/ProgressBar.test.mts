...
describe('ProgressBar', () => {
	...
	it('defaults to value 0', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) });
	...
	it('accepts initial value and max', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { value: 40, max: 200 });
		expect(pb.getValue()).toBe(40);
		expect(pb.getMax()).toBe(200);
	});

	it('clamps initial value to max', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { value: 999, max: 50 });
		expect(pb.getValue()).toBe(50);
	});

	it('setValue / getValue round-trip', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { max: 100 });
		pb.setValue(75);
	...
	it('setValue clamps above max', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { max: 50 });
		pb.setValue(100);
		expect(pb.getValue()).toBe(50);
	});
	...
	it('all cells are empty chars when value is 0', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(5, 1) }, { value: 0, showLabel: false });
		pb.render();
		for (let x = 0; x < 5; x++) {
	...
	it('all cells are filled chars when value equals max', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(5, 1) }, { value: 100, max: 100, showLabel: false });
		pb.render();
		for (let x = 0; x < 5; x++) {
			expect(pb.getCell(x, 0).char).toBe('█');
		}
	...
	it('half the bar is filled at 50%', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { value: 50, max: 100, showLabel: false });
		pb.render();
	...
	it('filled cells have the configured fillColor background', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(6, 1) }, { value: 100, max: 100, fillColor: 82, showLabel: false });
		pb.render();
		expect(pb.getCell(0, 0).attributes.background).toBe(82);
	});

	it('empty cells have the configured emptyColor background', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(6, 1) }, { value: 0, max: 100, emptyColor: 236, showLabel: false });
		pb.render();
		expect(pb.getCell(0, 0).attributes.background).toBe(236);
	});

	it('default fillColor is 75', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(4, 1) }, { value: 100, showLabel: false });
		pb.render();
		expect(pb.getCell(0, 0).attributes.background).toBe(75);
	});

	it('percentage label is centred when showLabel is true', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { value: 0, showLabel: true });
		pb.render();
	...
	it('no label chars when showLabel is false', () => {
		const pb = new ProgressBar({ pos: new Pos(0, 0), size: new Size(10, 1) }, { value: 0, showLabel: false });
		pb.render();
